...
const ushort _lookup[] = {0, 49345, 49537, 320, 49921, 960, 640, 49729, 50689, 1728, 1920, 51009, 1280, 50625, 50305, 1088, 52225, 3264, 3456, 52545, 3840, 53185, 52865, 3648,
...
+(NSString *) encode:(NSData *) inputData{
    ...
    Byte *input = (Byte*)malloc(len);
    memcpy(input, [inputData bytes], len);
    
    Byte *scratch = (Byte*)malloc(16);
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        {
            hash = [self hash:scratch withHash:hash];
    ...
    NSMutableData *data = [[NSMutableData alloc] initWithBytes:target length:16];
    free(input);
    free(scratch);
    free(target);
...
+(ushort) hash:(Byte *) scratch withHash:(ushort) hash{
    for (int i = 15; i >= 0; i--)
    {
        hash = (ushort)(
            hash >> 8 ^
